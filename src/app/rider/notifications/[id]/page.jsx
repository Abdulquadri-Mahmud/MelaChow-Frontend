...
export default function NotificationOrderDetails() {
    ...
    if (loading) {
        return (
            <div className="flex justify-center py-32 bg-[#0F1115] min-h-screen">
    ...
    if (!order) {
        return (
            <div className="flex flex-col items-center justify-center p-6 text-center pt-24 min-h-screen bg-[#0F1115]">
                <Package size={64} className="text-gray-600 mb-6" />
                <h1 className="text-2xl font-black text-white">Order Unavaliable</h1>
                <p className="text-gray-500 max-w-sm mt-3 font-medium">
                    This order may have been reassigned, cancelled, or doesn't exist.
                </p>
                <button
                    onClick={() => router.back()}
                    className="mt-8 px-6 py-3 bg-white/10 hover:bg-white/20 transition-colors rounded-full font-bold text-white shadow-xl flex gap-2 items-center"
                >
    ...
    return (
        <div className="min-h-screen pb-32 max-w-md mx-auto relative pt-4 text-white">
            {/* Header */}
            <div className="flex items-center gap-4 px-4 mb-6 sticky top-0 bg-[#0F1115]/90 backdrop-blur-xl z-10 py-4 border-b border-white/5">
                <button
                    onClick={() => router.back()}
                ...
                <div>
                    <h1 className="font-black text-xl leading-none mb-1">Assignment Info</h1>
                    <p className="text-xs font-bold text-orange-500 uppercase tracking-wider">#{String(order._id).slice(-6)}</p>
                </div>
            </div>
            ...
            <div className="px-4 space-y-4 relative z-0">
                ...
                <div className="relative pl-6 py-2">
                    ...
                    <motion.div initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.1 }} className="relative mb-6">
                        <div className="absolute -left-[22px] top-3 w-8 h-8 rounded-full bg-[#1A1D23] border-[3px] border-orange-500 flex items-center justify-center shadow-[0_0_15px_rgba(249,115,22,0.5)] z-10">
                            <Bike size={14} className="text-orange-500" />
                        </div>
                        <div className="bg-[#1A1D23] rounded-3xl p-5 border border-white/5 shadow-2xl ml-4">
                            <p className="text-[10px] uppercase font-black tracking-widest text-orange-500 mb-1">Pickup From</p>
                            <h3 className="font-black text-lg mb-1">{restaurant?.name || "GrubDash Vendor"}</h3>
                            <p className="text-gray-400 text-sm mb-4 leading-relaxed">{vendorAddress}</p>
                            <a
                                href={`https://maps.google.com/?q=${encodeURIComponent(vendorAddress)}`}
                                target="_blank"
                                rel="noopener noreferrer"
                                className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-white/5 hover:bg-white/10 font-bold text-xs"
                            >
                    ...
                    <motion.div initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.2 }} className="relative">
                        <div className="absolute -left-[22px] top-3 w-8 h-8 rounded-full bg-[#1A1D23] border-[3px] border-blue-500 flex items-center justify-center shadow-[0_0_15px_rgba(59,130,246,0.5)] z-10">
                            <MapPin size={14} className="text-blue-500" />
                        </div>
                        <div className="bg-[#1A1D23] rounded-3xl p-5 border border-white/5 shadow-2xl ml-4">
                            <p className="text-[10px] uppercase font-black tracking-widest text-blue-500 mb-1">Deliver To</p>
                            <h3 className="font-black text-lg mb-1">{customerName}</h3>
                            <p className="text-gray-400 text-sm mb-4 leading-relaxed line-clamp-2">{customerAddress}</p>
                            ...
                            <div className="flex gap-2">
                                <a
                                    ...
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="flex-1 inline-flex justify-center items-center gap-2 px-4 py-2 rounded-xl bg-white/5 hover:bg-white/10 font-bold text-xs"
                                >
                ...
                {/* Order Items */}
                <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.3 }} className="bg-[#1A1D23] rounded-3xl p-6 mt-4 border border-white/5">
                    ...
                    <ul className="space-y-4">
                        {order.items?.map((item, idx) => (
                            <li key={idx} className="flex gap-4 items-start">
                                <div className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center text-xs font-black text-white shrink-0 mt-0.5">
                                    {item.quantity}x
                                </div>
                                <div>
                                    <p className="font-bold text-sm text-gray-200">{item.variant?.name || "Order Item"}</p>
                                    {item.note && <p className="text-xs text-orange-400/80 mt-1 italic">"{item.note}"</p>}
                                </div>
                            </li>
                        ))}
                    </ul>
            ...
            <div className="fixed bottom-0 left-0 right-0 bg-[#0F1115]/90 backdrop-blur-2xl border-t border-white/10 p-4 z-50 rounded-t-[40px] shadow-[0_-10px_40px_rgba(0,0,0,0.5)] max-w-md mx-auto">
                        ...
                        >
                            {order.orderStatus === "rider_assigned" ? (
                                <button
                                    ...
                                    disabled={actionLoading}
                                    className="w-full h-16 rounded-[24px] bg-white text-orange-600 font-black tracking-wide flex items-center justify-center shadow-xl transition-all active:scale-95 disabled:opacity-50"
                                >
                                    {actionLoading ? <Loader2 className="animate-spin" size={24} /> : (
