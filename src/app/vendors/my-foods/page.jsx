...
export default function Page() {
  const { foods, isLoading, deleteFood } = useFoods();
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="bg-white dark:bg-[#1E293B] rounded-2xl p-5 border border-slate-200 dark:border-slate-800 shadow-sm">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-bold text-slate-700 dark:text-slate-300">Filter & Sort</span>
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="px-3 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm font-medium text-slate-700 dark:text-slate-300 focus:ring-2 focus:ring-[#FF6B00] outline-none transition-all"
            >
              <option value="all">All Categories ({foods?.data?.length || 0})</option>
              {categories.map((cat) => (
                <option key={cat} value={cat}>
                  {cat} ({foods?.data?.filter(f => f.category === cat).length})
                </option>
              ))}
            </select>
            {(statusFilter !== "all" || categoryFilter !== "all") && (
              <button
                ...
                className="text-xs text-slate-500 hover:text-[#FF6B00] font-medium flex items-center gap-1 transition-colors"
              >
                <X size={14} /> Clear
              </button>
            )}
          ...
          {tabs.map((tab) => {
            ...
            return (
              ...
              >
                ...
                <span className={`px-2 py-0.5 rounded-full text-[10px] ${isActive ? 'bg-white/20' : 'bg-slate-200 dark:bg-slate-700'}`}>
                  {count}
                </span>
              </button>
            )
